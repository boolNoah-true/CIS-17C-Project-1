 ...
 * Author: Noah Scott
 *
 * Created on December 17, 2023, 1:13 PM
 */
...
unordered_map<int, char> aiChoices;

//Global choice index for AI Data
int choiceIndex = 0;
...
void initAIChoices() {
    aiChoices[choiceIndex++] = { 'p' };  // AI chooses paper when the last human choice was rock
    aiChoices[choiceIndex++] = { 's' };  // AI chooses scissors when the last human choice was paper
    aiChoices[choiceIndex++] = { 'r' };  // AI chooses rock when the last human choice was scissors
}


void Game::aiData(char playerMove) {
   
   if (playerMove == 'r') {
        aiChoices[choiceIndex++] = 'p'; // Counter with paper
    } else if (playerMove == 'p') {
        aiChoices[choiceIndex++] = 's'; // Counter with scissors
    } else if (playerMove == 's') {
        aiChoices[choiceIndex++] = 'r'; // Counter with rock
    }
}
    ...
    char Game::aiChoice() {
        if (choiceIndex > 0) {
            int randomIndex = rand() % choiceIndex;
            return aiChoices[randomIndex];
        } else {
            static const char defaultChoices[] = {'r', 'p', 's'};
            return defaultChoices[rand() % 3];
        }
    }

int Game::playRound() {

    char input, artificialInput = aiChoice();
...
void writeToLeaderboard(const string& winner, int turns) {
...
int partition(vector<pair<int, string>>& leaderboard, int low, int high) {
    int pivot = leaderboard[high].first;
    int i = low - 1;
    for (int j = low; j <= high - 1; j++) {
        if (leaderboard[j].first < pivot) { // Notice the ">" for descending order
            i++;
...
void quickSort(vector<pair<int, string>>& leaderboard, int low, int high) {
    if (low < high) {
...
void sortAndSaveLeaderboard() {
    ...
    while (getline(file, line)) {
        ...
        string winner;
        if (!(ss >> turns)) {
            cerr << "Error: Invalid turn number in file." << endl;
            continue;
        }
        if (!(ss >> winner)) {
            cerr << "Error: Invalid winner name in file." << endl;
            continue;
        }
        leaderboard.push_back(make_pair(turns, winner));
    }
    file.close();

    // Convert size_t to int to avoid warning C4267
    int size = static_cast<int>(leaderboard.size());
    quickSort(leaderboard, 0, size - 1);
        ...
        for (int startY = 0; startY < SIZE; ++startY) {
            if (board[startX][startY] == aiPlayer) {
                ...
                for (const Move& move : potentialMoves) {
                    ...
                    memcpy(tempBoard, board, sizeof(char));
                    ...
                    // Undo the move
                    memcpy(board, tempBoard, sizeof(char));
...
void displayLeaderboard() {
    // First, sort and save the leaderboard
    sortAndSaveLeaderboard();

    // Then, read the sorted leaderboard and display it
    ifstream file("leaderboard.txt");
    if (!file) {
        cerr << "Unable to open leaderboard.txt" << endl;
        return;
    }
